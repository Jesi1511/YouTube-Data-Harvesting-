...
css = f"body {{ background-color: {background_color}; }}"


# Mongo Python connectivity
mongo_client = pm.MongoClient('mongodb://localhost:27017')
db = mongo_client['Youtube_data_Jc']
collection = db['YouTube']
#collection = db['channel_detail']

# Sql Python Connectivity
db_config = pg2.connect(host="localhost",
                                user="postgres",
...
#api_key ="AIzaSyCs-7JyHsw13JF6o39ORyWEKgem8wEbTG0" # provide your api key here
#api_key ="AIzaSyCqvovDKf_LsFjYwylsJ83FO6izlPq7K24"
api_key ="AIzaSyC4w9XGxhu51-E33-3le5hjdbdG3IEQ9v8"
...
#--------------------------------------------------------------------------------------------------------
    ...
    def datatransform(self, cd, pl, vd, cod):
        ...
        for i in range(len(video["Duration"])):
            duration = parse_duration(video["Duration"].loc[i])
...
st.set_page_config(page_title='YouTube Project By Jesima Parvin',layout="wide"   # Page Title
 )
...
if selected == 'INTRO':
            ...
            if len(Names)>0:
                ...
                if Channel_name not in sql_chan_names:
                    if Channel_name in Names:
                        ...
                        final_vd =f_vd.reindex(columns=['video_id','channel_id','channel_name','comment_count','description','dislike_count','duration','favorite_count','like_count','playlist_id','published_date','title','view_count','year_publishedat'])
    ...
    st.header(":green[Linkedin]: [Check out](https://www.linkedin.com/in/jesima-parvin/)")
    st.header(":green[View More] Projects : [Quick](https://github.com/Jesi1511)")
